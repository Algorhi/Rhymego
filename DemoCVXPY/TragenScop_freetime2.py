...
x0  = 0; y0  = 0;  z0  = 0
xf  = 400;yf  = 400;zf  = 400
psi0 = 20*np.pi/180;phi0 = 40*np.pi/180
psif = 40*np.pi/180;phif = 80*np.pi/180
Vconst = 10;rate_max = 5*np.pi/180
Vx0 = Vconst*np.sin(psi0);Vxf=Vconst*np.sin(psif)
Vy0 = Vconst*np.cos(psi0)*np.cos(phi0);Vyf=Vconst*np.cos(psif)*np.cos(phif)
Vz0 = Vconst*np.cos(psi0)*np.sin(phi0);Vzf=Vconst*np.cos(psif)*np.sin(phif)
N = 100;tau = 0.21;S = 5;
S = S+1
